...
struct machine_t {
  ...
  // static std::optional<machine_t> load_binary(const std::filesystem::path&
  // path,
 ...
 public:
  ...
  uint32_t              _mode        = 0b11;
  std::function<void()> _pre_decode_callback;
  std::function<void(riscv::instruction_t)> _post_decode_callback;
};
